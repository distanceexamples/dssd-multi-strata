...
description: |
  Designing distance sampling surveys containing more than one stratum.
author:
  - name: Laura Marshall 
...
# Here I set a seed so that my text matches the particular survey generated.
# Do not set seeds when you want to randomly generate sets of transects!
set.seed(724)
```
...
Additional information on how to manually create multi-strata study regions inside R can be found in the [manual creation of study areas appendix](#manual-creation-of-multiple-strata-study-areas). 

## Creating a Coverage Grid

The coverage grid can be created in the same way as for a single stratum study region. The creation of the coverage grid involves creating a single grid of points over the entire study area and so is unaffected by the region being subdivided into multiple strata, Figure \@ref(fig:covergrid).

```{r covergrid, fig.width=6, fig.align='center', fig.asp=1, fig.cap="Coverage grid with approximately 1000 grid points."}
...
```{r defaultdesign, fig.asp = 1, fig.width=6, fig.align='center', fig.cap="Survey generated from on a multi-strata default design with stratum specific design angles. "}
...
We can also view a summary of the survey shown in Figure \@ref(fig:defaultdesign). The output shown below indicates that `dssd` selected a spacing of around 17.9 km (17946.09 m) between the systematic parallel lines. As both strata used the same systematic design the spacing values were calculated globally and are identical for both strata. It also tells us that the percentage of the study region covered was around 22% in both strata and that for this particular survey we have achieved exactly 20 transects, 11 in the northern strata and 9 in the southern strata.
...
```{r zzspcov, echo = FALSE, fig.asp = 1, fig.width=5, fig.align='center', fig.cap="Plot of the coverage scores for the mixed type design, based on the generation of 500 surveys from this design."}
# If you want to have a look at my coverage simulation results you can load
# them from the package using the following code:
...
We can then have a look at the coverage scores to examine coverage in the two strata. We observe on average the coverage is higher in the northern stratum than the southern stratum because spacing was half as large in the northern stratum, Figure \@ref(fig:segdesign3).
...
```{r segdesign3, fig.asp = 1, echo = FALSE, fig.width=5, fig.align='center', fig.cap="Coverage scores plotted for the segmented grid design."}
# If you want to have a look at coverage simulation results, load
# them from the package using the following code:
...
When there are differences in average coverage between strata it is sometimes difficult to visualise non-uniform coverage within strata. The survey design package also provides the functionality to view the coverage score plot for individual strata, Figures \@ref(fig:segdesign4) and \@ref(fig:segdesign5). While the minor edge effects in the northern stratum were apparent when the study area was plotted as a whole, Figure \@ref(fig:segdesign3), the edge effects in the southern stratum were not detectable in this plot. However, we can see from Figure \@ref(fig:segdesign5) that the edge effects in the southern stratum are also very small in comparison to the stratum area.

```{r segdesign4, fig.asp = 1, fig.width=4.5, fig.align='center', fig.cap="Coverage scores plotted only for the northern stratum."}
plot(design, strata.id = 1, subtitle = "Coverage Northern Strata")
```

```{r segdesign5, fig.asp = 1, fig.width=4.5, fig.align='center', fig.cap="Coverage scores plotted only for the southern stratum."}
...
For this point transect example we will use a different study region. This is an area of forest in Scotland between Dundee and St Andrews made up of two strata, Figure \@ref(fig:pointregion). As this region is not projected we will first need to project the study area onto a flat plane.

```{r pointregion, fig.align='center', fig.width=6, fig.height = 4.5, fig.cap = "Study region depicting an area of forest between Dundee and St Andrews in Scotland. It comprises of a main stratum and a Morton Loch stratum which forms part of a nature reserve in the area."}
...
Point transect designs and point transect design parameters can be varied across strata in the same way as line transect designs. Here we use a systematic grid of point in both the main and Morton Lochs strata. We specify that there should be approximately 80 samplers across both strata and that we would like 25% of the effort allocated to the Morton Lochs strata and 75% to the main strata. As the Morton Lochs strata is such a small area it could be prone to edge effects so we will use a plus sampling strategy. Figure \@ref(fig:pointeg) shows one possible realisation of this design.  

```{r pointeg, fig.asp = 1, fig.width=6, fig.align='center', fig.cap="An example multi strata point transect survey."}
...
```{r makeown, fig.asp = 1, fig.width=6, fig.align='center', fig.cap="A single study region with multiple polygon parts."}
...
```{r makeown2, fig.asp = 1, fig.width=6, fig.height = 4, fig.align='center', fig.cap="A single study region with multiple polygon parts.  Each are separate strata."}
outer <- matrix(c(0,0,15,0,15,10,0,10,0,0),ncol=2, byrow=TRUE)
hole1 <- matrix(c(2,2,2,3,3,3,3,2,2,2),ncol=2, byrow=TRUE)
hole2 <- matrix(c(5,5,5,6,7,6,8,5.5,7,5,5,5),ncol=2, byrow=TRUE)
